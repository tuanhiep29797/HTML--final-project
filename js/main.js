...
//login
localStorage.setItem("admin", JSON.stringify({email: "admin@gmail.com", password: "admin123"}));
    ...
    products.forEach((item, index) => {
        const row = `
            <tr>
                ...
                <td>
                    <button onclick="editProduct(${item.id})" class="btn" popovertarget="addOrUpdate"> Edit</button>
                    <button onclick="deleteProduct(${item.id})" class="btn"> Delete</button>
                </td>
            </tr>
        `
        tbody.innerHTML += row;
